...
namespace AzureFunctionConsumer
{
    class Program
    {
        ...
        private static async Task MainHTTPTriggerAsync(string[] args)
        {
            ...
            for (int i = 0; i < HTTPRequestsToSend; i++)
            {
                 SendHTTPRequests(HTTPTriggerUrl, HTTPTriggerFunctionKey, Name);
            }
        }
        private static async Task SendHTTPRequests(string Url, string FunctionKey, string Name)
        {
            try
            {
